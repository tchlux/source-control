...
no unstaged changes, it shows staged changes. Pass multiple paths after `--` to
specify literal paths to show (and avoid treating numeric names as commit IDs).
Diffs include three unchanged context lines around changes and omit output above
100 diff lines unless `--full` is used. Historical diffs compare commit endpoints;
for example, `sc diff 2 --against 1` shows changes from commit `1` to commit `2`.
Use `--stat` to print one changed-file summary row and total insertion/deletion
counts instead of diff hunks.
