...
//   sc diff [--full|--stat] [--] [paths...]
//   sc diff [--full|--stat] --staged [--] [paths...]
//   sc diff [--full|--stat] --latest [--] [paths...]
//   sc diff [--full|--stat] COMMIT [--against COMMIT]
//   sc diff [--full|--stat] --against COMMIT [COMMIT]
//   sc commit -m "message"
//   sc amend [-m "message"]
//   sc log [path]
...
const char * diff_kind(int old_type, int old_exists, int new_type, int new_exists) {
  return ! old_exists && new_exists ? "added" :
    (old_exists && ! new_exists ? "deleted" :
      (old_type != new_type ? "type-change" : (old_type == SOURCE_LINK ? "link-change" : "modified")));
}

const char * diff_stat_kind(bytes_t * old, int old_type, int old_exists, bytes_t * new, int new_type, int new_exists) {
  if ((old_exists && old_type == SOURCE_FILE && binary_like(old)) ||
      (new_exists && new_type == SOURCE_FILE && binary_like(new))) return "binary";
  return diff_kind(old_type, old_exists, new_type, new_exists);
}

void print_stat_counts(diff_stat_t * stat, int added, int deleted) {
  stat->files++;
  stat->added += added;
...
int print_one_stat(const char * path, bytes_t * old, int old_type, int old_exists, bytes_t * new, int new_type, int new_exists, diff_stat_t * stat) {
  if (bytes_equal(old, old_type, old_exists, new, new_type, new_exists)) return 0;
  int added = 0, deleted = 0;
  printf("%s | ", path);
  if (status_line_counts(old, old_type, old_exists, new, new_type, new_exists, &added, &deleted))
    print_stat_counts(stat, added, deleted);
  else {
    stat->files++;
    printf("%s\n", diff_stat_kind(old, old_type, old_exists, new, new_type, new_exists));
  }
  return 1;
}

int print_move_stat(const char * old_path, const char * new_path, bytes_t * old, int old_type, int old_exists, bytes_t * new, int new_type, int new_exists, diff_stat_t * stat) {
  int added = 0, deleted = 0;
  printf("%s -> %s | ", old_path, new_path);
  if (status_line_counts(old, old_type, old_exists, new, new_type, new_exists, &added, &deleted))
    print_stat_counts(stat, added, deleted);
  else {
    stat->files++;
...
void print_stat_total(diff_stat_t * stat) {
  if (! stat || ! stat->files) return;
  printf("%d file%s changed, %d insertion%s(+), %d deletion%s(-)\n",
    stat->files, stat->files == 1 ? "" : "s",
    stat->added, stat->added == 1 ? "" : "s",
    stat->deleted, stat->deleted == 1 ? "" : "s");
}
...
int diff_path_matches(paths_t * filters, const char * path) {
  if (! filters || filters->n == 0) return 1;
  for (int i = 0; i < filters->n; i++)
    if (strcmp(filters->v[i], path) == 0) return 1;
  return 0;
}

int diff_add_filter(repo_t * repo, paths_t * filters, const char * arg) {
  char path[PATH_MAX];
  if (rel_path(repo, arg, path)) normalize_filter_path(path);
  else { snprintf(path, sizeof(path), "%s", arg); normalize_filter_path(path); }
  return paths_push_unique(filters, path);
}

int diff_local(repo_t * repo, index_t * idx, stage_t * stage, paths_t * filters, int full, diff_stat_t * stat) {
  int shown = 0;
  ...
  for (int i = 0; i < idx->n; i++) {
    ...
    if (! diff_path_matches(filters, e->path) && (! move || ! diff_path_matches(filters, move->path)) &&
        (work_move < 0 || ! diff_path_matches(filters, work_paths.v[work_move]))) continue;
    progress_tick(&progress, filters && filters->n ? -1 : idx->n, e->path);
    bytes_t old = {0}, new = {0};
    ...
    if (work_move >= 0) {
      ...
      if (stat) shown += print_move_stat(e->path, work_paths.v[work_move], &old, old_type, old_exists, &new, new_type, new_exists, stat);
      else print_move_diff(e->path, work_paths.v[work_move], &old, old_type, old_exists, &new, new_type, new_exists, &shown, full);
      free_bytes(&old);
      free_bytes(&new);
      continue;
    }
    if (move) {
      ...
      if (stat) shown += print_one_stat(move->path, &old, old_type, old_exists, &new, new_type, new_exists, stat);
      else print_one_diff(move->path, &old, old_type, old_exists, &new, new_type, new_exists, &shown, full);
      free_bytes(&old);
      free_bytes(&new);
      continue;
    }
    ...
    if (stat) shown += print_one_stat(e->path, &old, old_type, old_exists, &new, new_type, new_exists, stat);
    else print_one_diff(e->path, &old, old_type, old_exists, &new, new_type, new_exists, &shown, full);
    free_bytes(&old);
    free_bytes(&new);
  }
  progress_done(&progress);
...
int diff_staged(repo_t * repo, index_t * idx, stage_t * stage, paths_t * filters, int full, diff_stat_t * stat) {
  int shown = 0;
  progress_t progress;
  progress_start(&progress, "diff");
  for (int i = 0; i < stage->n; i++) {
    stage_entry_t * se = &stage->v[i];
    if (! diff_path_matches(filters, se->path) && (! se->old_path || ! diff_path_matches(filters, se->old_path))) continue;
    progress_tick(&progress, filters && filters->n ? -1 : stage->n, se->path);
    ...
    if (se->action == STAGE_MOVE && se->old_path) {
      if (stat) shown += print_move_stat(se->old_path, se->path, &old, old_type, old_exists, &new, new_type, new_exists, stat);
      else print_move_diff(se->old_path, se->path, &old, old_type, old_exists, &new, new_type, new_exists, &shown, full);
    } else {
      if (stat) shown += print_one_stat(se->path, &old, old_type, old_exists, &new, new_type, new_exists, stat);
      else print_one_diff(se->path, &old, old_type, old_exists, &new, new_type, new_exists, &shown, full);
    }
    free_bytes(&old);
    free_bytes(&new);
  }
...
int diff_against(repo_t * repo, index_t * idx, unsigned long long target, paths_t * filters, int full, diff_stat_t * stat) {
  paths_t paths = {0};
  int shown = 0;
  int ok = 1;
  ...
  progress_start(&progress, "diff");
  for (int i = 0; i < paths.n; i++) {
    if (! diff_path_matches(filters, paths.v[i])) continue;
    progress_tick(&progress, filters && filters->n ? -1 : paths.n, paths.v[i]);
    bytes_t old = {0}, new = {0};
    int old_type = SOURCE_NONE, new_type = SOURCE_NONE, old_exists = 0, new_exists = 0;
    ...
    if (stat) shown += print_one_stat(paths.v[i], &old, old_type, old_exists, &new, new_type, new_exists, stat);
    else print_one_diff(paths.v[i], &old, old_type, old_exists, &new, new_type, new_exists, &shown, full);
    free_bytes(&old);
    free_bytes(&new);
  }
  progress_done(&progress);
...
int diff_commits(repo_t * repo, index_t * idx, unsigned long long base, unsigned long long target, paths_t * filters, int full, int base_root, diff_stat_t * stat) {
  paths_t paths = {0};
  int shown = 0;
...
int diff_latest(repo_t * repo, paths_t * filters, int full, diff_stat_t * stat) {
  unsigned long long head = 0;
  commit_t c = {0};
  int shown = 0;
  ...
  for (int i = 0; i < c.n; i++) {
    ...
    if (! diff_path_matches(filters, f->path)) continue;
    progress_tick(&progress, filters && filters->n ? -1 : c.n, f->path);
    bytes_t old = {0}, new = {0};
    ...
    if (stat) shown += print_one_stat(f->path, &old, old_type, old_exists, &new, new_type, new_exists, stat);
    else print_one_diff(f->path, &old, old_type, old_exists, &new, new_type, new_exists, &shown, full);
    free_bytes(&old);
    free_bytes(&new);
  }
  progress_done(&progress);
...
int command_diff(repo_t * repo, int argc, char ** argv) {
  ...
  stage_t stage = {0};
  paths_t filters = {0};
  const char * usage = "usage: sc diff [--full|--stat] [--staged|--latest] [--] [paths...] [COMMIT] [--against COMMIT]\n";
  int latest = 0, against_set = 0, full = 0, stat_mode = 0, staged = 0, target_set = 0, path_mode = 0;
  unsigned long long against = 0, target = 0;
  for (int i = 0; i < argc; i++) {
    if (path_mode) {
      if (! diff_add_filter(repo, &filters, argv[i])) goto oom;
    } else if (strcmp(argv[i], "--") == 0) path_mode = 1;
    else if (strcmp(argv[i], "--latest") == 0) latest = 1;
    else if (strcmp(argv[i], "--full") == 0) full = 1;
    else if (strcmp(argv[i], "--stat") == 0) stat_mode = 1;
    else if (strcmp(argv[i], "--staged") == 0) staged = 1;
    else if (strcmp(argv[i], "--against") == 0) {
      if (i + 1 >= argc) { fprintf(stderr, "%s", usage); paths_free(&filters); return 1; }
      against_set = 1;
      if (! diff_commit_ref(repo, argv[++i], &against, 1)) { fprintf(stderr, "unknown commit: %s\n", argv[i]); paths_free(&filters); return 1; }
    } else {
      if (argv[i][0] == '-') { fprintf(stderr, "%s", usage); paths_free(&filters); return 1; }
  ...
  if (! load_index(repo, &idx)) { paths_free(&filters); return 1; }
  if (! load_stage(repo, &stage)) { paths_free(&filters); index_free(&idx); return 1; }
  diff_stat_t stat = {0};
  diff_stat_t * statp = stat_mode ? &stat : NULL;
  int rc = latest ? diff_latest(repo, &filters, full, statp) :
    (target_set ? diff_commits(repo, &idx, against, target, &filters, full, base_root, statp) :
      (against_set ? diff_against(repo, &idx, against, &filters, full, statp) : 0));
  if (staged) diff_staged(repo, &idx, &stage, &filters, full, statp);
  else if (! latest && ! against_set && ! diff_local(repo, &idx, &stage, &filters, full, statp) && stage.n)
    diff_staged(repo, &idx, &stage, &filters, full, statp);
  if (! rc) print_stat_total(statp);
  stage_free(&stage);
  index_free(&idx);
  paths_free(&filters);
  return rc;
...
void print_help(void) {
  printf(
    ...
    "  sc diff [--full|--stat] --staged [--] [paths...]\n"
    "  sc diff [--full|--stat] --latest [--] [paths...]\n"
    "  sc diff [--full|--stat] COMMIT [--against BASE]\n"
    "  sc diff [--full|--stat] --against BASE [COMMIT]\n"
    "  sc commit -m \"message\"\n"
    "  sc amend [-m \"message\"]\n"
    "  sc log [path]\n"
    "  sc browse [COMMIT] [FILE]\n"
